...
publisher_colours <- c("Elsevier" = "#e43d04",
                       "Wiley" = "black",
...
```{r}
# HERE (Add abbrev to pubs_summary)
...
pubs_all <-
  ...
  mutate(
    publisher_clean = case_when(
      ...
      str_detect(publisher, "IEEE") ~ "Institute of Electrical and Electronics Engineers",
...
pubs_all |> 
  ...
  fmt_percent(columns = pc, decimals = 0) |> 
  tab_options(column_labels.hidden = TRUE,
              table.background.color = "#2d2d2d",
              table.font.color.light = "white")
```
...
```{r}
# Get country codes for each pubisher (add to original query)

# Get unique publisher ids

publisher_ids <- unique(pubs_all$publisher_id)

# Strip publisher ids to only the ID



# Query OpenAlex for the country code

req_publishers <-
  request("https://api.openalex.org/") |> 
  req_url_path_append("publishers") |> 
  req_url_query(search = ) |> 
  req_url_query(select = c("id", "display_name", "country_codes"), .multi = "comma") |> 
  req_url_query(mailto = Sys.getenv("EMAIL_ADDRESS")) |> 
  req_perform() |>
  resp_body_json()

# Process into df and join to pubs_all

test <-
  req_publishers$results |> 
  map(as.data.frame) |> 
  bind_rows() |> 
  left_join()
...
```{r}
sum(duplicated(pubs_all$item_id)) / length(pubs_all$item_id)

# Add column to denote works with more than one NZ affiliation
...
bind_rows(works_all, works_collab)
