...
oa_data <- read.csv("data/oa-data.csv")
```
...
ggplot(trend_nz, aes(x = year, y = pc)) +
  ...
  ylab("Open Access (%)") +
  ylim(0, 80) +
  scale_x_continuous(n.breaks = prev_year-min(oa_data$year)) +
  plot_theme()
```
...
```{r}
plot_inst <- function(oa_data) {
  p <- ggplot(oa_data, aes(x = year, y = pc, colour = key)) +
    geom_line(size = 1) +
    scale_oa +
    xlab("") +
    ylab("Proportion (%)") +
    ylim(0, 80) +
    scale_x_continuous(n.breaks = prev_year-min(oa_data$year)) +
    ggtitle(paste0(unique(oa_data$institution), 
                   ", ", min(oa_data$year), "-", prev_year)) +
    plot_theme()
  return(p)
}

plot_nz_list <- oa_data |>
  filter(country == "nz") |> 
  group_by(institution, .add = TRUE) |> 
  group_split(institution) |> 
  map(~ plot_inst(.x))

plot_au_list <- oa_data |>
  filter(country == "au") |> 
  group_by(institution, .add = TRUE) |> 
  group_split(institution) |> 
  map(~ plot_inst(.x))
