...
pubs_all |>
  ...
  ylab("") +
  scale_y_continuous(labels = scales::label_percent()) +
  scale_fill_manual(values = publisher_colours[1:5]) +
  plot_theme()
...
#     pc = n/ sum(n)
#   ) |>
#   ggplot(aes(x = publisher_clean, y = pc, fill = field)) +
...
#     pc = n/ sum(n)
#   ) |>
#   ggplot(aes(x = field, y = pc)) +
#   geom_bar(stat = "identity", show.legend = FALSE) +
#   coord_flip() +
#   xlab("") +
#   ylab("") +
#   scale_y_continuous(labels = scales::label_percent()) +
#   facet_wrap(vars(publisher_clean), nrow = 1)
#   plot_theme()
...
left_join(journal_rates, journal_citations) |> 
  rename(Journal = journal,
         Articles = articles,
         "Percent OA" = percent_open,
         "Median Citations" = median_citations) |>
  select(-is_oa, -n, -citations) |> 
  relocate(c(Articles, "Percent OA", "Median Citations"), .after = Journal) |>
  arrange(desc(Articles)) |>
  mutate(
    "Article Share" = Articles / nz_articles
  ) |> 
  ...
  fmt_number(columns = c(Articles, "Median Citations"), 
             sep_mark = ",", decimals = 0) |> 
  fmt_percent(columns = "Percent OA", decimals = 0) |> 
  fmt_percent(columns = "Article Share", decimals = 2) |> 
  tab_options(table.background.color = "#2d2d2d",
              table.font.color.light = "white")
```
